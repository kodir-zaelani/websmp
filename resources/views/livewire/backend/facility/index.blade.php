<div>
    @section('title', 'List of Facility')
                ...
                <div class="d-inline-block align-items-center">
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page">Facility</li>
                        </ol>
                    </nav>
                </div>
            </div>
                ...
                <div class="box box-bordered border-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-controls pull-right">
                            <a class="btn btn-xs btn-primary" href="{{ route('backend.facility.create') }}"><i
                            ...
                            <div class="col-xl-12 col-md-12 col-lg-12 col-12">
                                @if ($selectPage)
                                    <div class="mb-2 col-md-10">
                                        @if ($selectAll)
                                            <div>
                                                You have selected all <strong>{{ $datafacilityall->total() }}</strong>
                                                items.
                                            </div>
                                        @else
                                            <div>
                                                ...
                                                want to Select All
                                                <strong>{{ $datafacilityall->total() }}</strong>?
                                ...
                                @if ($datafacilityall->count())
                                    <div class="table-responsive">
                                        <table class="table mb-0 table-hover">
                                            ...
                                            <tbody>
                                                @foreach ($datafacilityall as $no => $item)
                                                    <tr class="@if ($this->isChecked($item->id)) table-primary @endif">
                                                        <th class="text-right" scope="row">
                                                            {{ $no + $datafacilityall->firstItem() }}</th>
                                                        {{-- <td>
                                                    <div class="checkbox">
                                                        <input type="checkbox" id="Checkbox_{{ $no + $datafacilityall->firstItem() }}" value="{{ $item->id }}" wire:model="checked">
                                                        <label for="Checkbox_{{ $no + $datafacilityall->firstItem() }}"></label>
                                                    </div>
                                                </td> --}}
                                                        <td width="10%">
                                                        ...
                                                        <td class="text-center align-midle">
                                                            ...
                                                            <a title="Edit" class="btn btn-xs btn-warning edit-row"
                                                                href="{{ route('backend.facility.edit', $item->id) }}">
                                    ...
                                    <div class="mt-3 row">
                                        <div class="col-xl-7 col-md-7 col-lg-7 col-12 ">
                                            {{ $datafacilityall->links() }}
                                        </div>
                                        <div class="text-center col-xl-5 col-md-5 col-lg-5 col-12">
                                            Page : {{ $datafacilityall->currentPage() }} | Show
                                            {{ $datafacilityall->count() }} data of {{ $datafacilityall->total() }}
                                        </div>
                                    </div>
                                @else
                                    <hr>
